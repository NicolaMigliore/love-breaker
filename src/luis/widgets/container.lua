...
function Container.setluis(luisObj)
	luis = luisObj
end

-- Container
function Container.new(width, height, row, col, isDraggable, isResizable, customTheme, containerName)
	local containerTheme = customTheme or luis.theme.container
	local container = {
		type = "Container",
		name = containerName or "Container", -- optional, makes it better when handling multiple containers
		...
		isDragging = false,
		isResizable = isResizable or false,
		isResizing = false,
		...
		--     return false
		-- end,
		addChild = function(self, child, relativeY, relativeX)
		...
		arrangeChildren = function(self)
			...
			for _, child in ipairs(self.children) do
				...
				child.position.x = self.position.x + (child.relativeX - 1 or 0) * luis.gridSize
				child.position.y = self.position.y + (child.relativeY - 1 or 0) * luis.gridSize
		...
		defaultDraw = function(self)
			...
			if self.isResizable then
				...
				love.graphics.rectangle("fill", self.position.x + self.width - containerTheme.handleSize,
					self.position.y + self.height - containerTheme.handleSize, containerTheme.handleSize,
					containerTheme.handleSize, containerTheme.cornerRadius)
			end
			...
			for _, child in ipairs(self.children) do
				...
				if luis.showElementOutlines then
					...
					love.graphics.print(child.type, child.position.x + child.width / 2 - string.len(child.type) * 2,
						child.position.y)
			...
			if luis.showElementOutlines then
				...
				love.graphics.setColor(1, 1, 1, 0.7)
				love.graphics.setFont(luis.theme.system.font)
				love.graphics.print(self.name, self.position.x + self.width / 2 - string.len(self.name) * 2,
			...
			if self.internalFocusActive and self.currentChildFocusIndex and self.currentChildFocusIndex > 0 then
				...
				love.graphics.rectangle("line", focusedChild.position.x - 2, focusedChild.position.y - 2,
					focusedChild.width + 4, focusedChild.height + 4, containerTheme.cornerRadius)
			end
		end,
		...
		click = function(self, x, y, button, istouch)
			...
			if self:isInResizeHandle(x, y) and self.isResizable then
				self.isResizing = true
				return true
			elseif self:isInContainer(x, y) and self.isDraggable then
				print('clicking')
				for _, child in ipairs(self.children) do
