...
function button.new(text, width, height, onClick, onRelease, row, col, customTheme)
	...
	return {
		...
		defaultDraw = function(self)
			drawElevatedRectangle(self.position.x, self.position.y, self.width, self.height,
				{ self.colorR, self.colorG, self.colorB, self.colorA }, self.elevation, buttonTheme.cornerRadius)

			-- Draw text
			love.graphics.setColor(buttonTheme.textColor)
			love.graphics.setFont(luis.theme.text.font)
			love.graphics.printf(self.text, self.position.x,
				self.position.y + (self.height - luis.theme.text.font:getHeight()) / 2, self.width, buttonTheme.align)
