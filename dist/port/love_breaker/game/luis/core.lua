...
function luis.createElement(layerName, widgetType, ...)
	...
	if widgetType == "FlexContainer" and type((...)) == "table" and (...).type == "FlexContainer" then
		...
		if luis["new" .. widgetType] then
			print('widget ' .. widgetType .. ' type ' .. type((...)))
...
function luis.drawElementOutline(element)
	...
	local text = element.position.x / luis.gridSize + 1 ..
	" x "                                                 -- we have to add +1 as the grid is indexed at 1,1 not 0,0 !!
	love.graphics.print(text, element.position.x + 2, element.position.y)
	love.graphics.print(element.position.y / luis.gridSize + 1, element.position.x + string.len(text) * 4 + 12,
		element.position.y)

	love.graphics.print(element.width / luis.gridSize, element.position.x + element.width - 22, element.position.y)
	love.graphics.print(element.height / luis.gridSize, element.position.x + element.width - 22,
...
function luis.draw()
	...
	if luis.showMetrics then
		...
		love.graphics.rectangle("fill", 5, luis.baseHeight - (#luis.stats * 20) - 15, maxWidth + 10,
			(#luis.stats * 20) + 10)
